...
const OBJ_DIR = 'models/程阳永济桥带贴图模型/'
const OBJ_NAME = '程阳永济桥.obj'
const MTL_NAME = '程阳永济桥.mtl'

const BASE_STEP = 1
const BRIDGE_SPAN = 4
const SHADER_VERSION = 11 // 递增强制重建 ShaderMaterial（HMR 时状态保留需手动触发）

// 背景图片路径（通过 inline style 覆盖 sq-* CSS 中的 image）
const BG_INITIAL = 'url(images/程阳永济桥初始背景图.webp)'
const BG_NO_BRIDGE = 'url(images/程阳永济桥无桥背景图.webp)'
const BG_PNG = 'url(images/程阳永济桥-桥抠图.webp)'
const PNG_SRC = 'images/程阳永济桥-桥抠图.webp'
...
const DEFAULT_TARGET = new THREE.Vector3(-2.33, 0.20, -0.19)
...
// ═══════════════════════════════════════════════════════════
// TexturedModel — ShaderMaterial 贴图增强（绕过 PBR 黑暗，等同手电筒提亮效果）
// ═══════════════════════════════════════════════════════════
...
function TexturedModel({
  ...
  const [transform, setTransform] = useState({ minY: -2, maxY: 2, scale: 1, cx: 0, cy: 0, cz: 0 })
  const loadedRef = useRef(false)

  // 手动加载：先 MTL(preload 纹理) → 再 OBJ → 替换所有材质为贴图增强 ShaderMaterial
  useEffect(() => {
    ...
    mtlLoader.load(MTL_NAME, (materials) => {
      ...
      objLoader.load(OBJ_NAME, (loadedObj) => {
          ...
          if ((child as THREE.Mesh).isMesh) {
            ...
            oldMats.forEach((oldMat) => {
              ...
              const shaderMat = new THREE.ShaderMaterial({
                ...
                vertexShader: TEXTURED_ENHANCED_VERTEX,
                fragmentShader: TEXTURED_ENHANCED_FRAGMENT,
                transparent: true,
                depthWrite: opacity > 0.9,
              })
              replacements.push(shaderMat)
              newMats.push(shaderMat)
            })

            if (replacements.length === 1) {
              mesh.material = replacements[0]
        ...
        if (minY === Infinity) {
          setTransform({ minY: -2, maxY: 2, scale: 1, cx: 0, cy: 0, cz: 0 })
        } else {
          ...
          setTransform({ minY, maxY, scale, cx, cy, cz })
        }
  ...
  }, [SHADER_VERSION, onLoad, onProgress])

  // 每帧更新 ShaderMaterial uniforms + dissolve clipping plane
  useFrame(() => {
    const { minY, maxY } = transform
    for (const mat of matRefs.current) {
      if (mat.uniforms) {
        mat.uniforms.uOpacity.value = opacity
        mat.uniforms.uBrightness.value = 0.25
        mat.depthWrite = opacity > 0.9
      }
      if (dissolveProgress > 0) {
        const clipY = minY + (maxY - minY) * dissolveProgress
        mat.clippingPlanes = [new THREE.Plane(new THREE.Vector3(0, -1, 0), clipY)]
...
// ═══════════════════════════════════════════════════════════
// 贴图增强着色器 — 绕过 PBR 直接输出贴图 + 亮度增强（类手电筒效果）
// ═══════════════════════════════════════════════════════════

const TEXTURED_ENHANCED_VERTEX = /* glsl */ `
varying vec2 vUv;
void main() {
  vec4 mvPosition = modelViewMatrix * vec4(position, 1.0);
...
const TEXTURED_ENHANCED_FRAGMENT = /* glsl */ `
varying vec2 vUv;
uniform sampler2D uMap;
...
void main() {
  vec4 texColor = texture2D(uMap, vUv);
  // Uniform 亮度增强 — 等同手电筒光斑内 glow=1.25 但全域生效
  float glow = 1.0 + uBrightness;
...
function FlashlightModel({
  ...
  // 每帧更新鼠标位置 + aspect + radius uniform（radius 通过 useFrame 更新确保 HMR 生效）
  useFrame(() => {
    ...
    for (const mat of matRefs.current) {
      if (mat.uniforms) {
        ...
        mat.uniforms.uRadius.value = 0.40
        mat.uniforms.uFeather.value = 0.55
      }
    }
  })
...
function ParticleCloud({
      ...
      if ((child as any).geometry) {
        ...
        for (let i = 0; i < posAttr.count; i += BASE_STEP) {
          ...
          if (ny > 0.6) {
            ...
            keepProb = 0.85     // 悬梁/中层 → 保留85%
          } else {
            keepProb = 0.95     // 基座/下层 → 保留95%
          }
  ...
  const pointsMat = useMemo(() => {
    const mat = new THREE.ShaderMaterial({
      ...
      vertexShader: /* glsl */ `
        ...
        void main() {
          vec4 mvPosition = modelViewMatrix * vec4(position, 1.0);
          gl_PointSize = 0.028 * (300.0 / -mvPosition.z);
          gl_Position = projectionMatrix * mvPosition;
          vColor = color;
  ...
  // 每帧同步 mouse + aspect + flashlight + opacity
  useFrame(() => {
    if (!pointsMat.uniforms) return
    pointsMat.uniforms.uOpacity.value = opacity
...
function ThreeScene({
  ...
  return (
    <>
      ...
      {/* 态B — 贴图模型 (始终渲染以预加载，visible控制显示避免点击时等待加载) */}
      <group
        ref={modelGroupRef}
        position={calibrateMode ? modelPos : [-1.54, -0.02, -0.31]}
        rotation={calibrateMode ? modelRot : [0, 0, 0]}
        scale={calibrateMode ? [modelScale, modelScale, modelScale] : [1.19, 1.19, 1.19]}
        renderOrder={0}
      ...
      <group position={[-1.54, -0.02, -0.31]} rotation={[0, 0, 0]} scale={[1.19, 1.19, 1.19]}>
      ...
      <group
        ref={partGroupRef}
        position={calibrateMode ? partPos : [-1.54, -0.02, -0.31]}
        rotation={calibrateMode ? partRot : [0, 0, 0]}
        scale={calibrateMode ? [partScale, partScale, partScale] : [1.19, 1.19, 1.19]}
        renderOrder={1}
      >
        <ParticleCloud
...
export default function ChengyangYongjiBridge3D({ bridge, onClose, actionsRef, onLoad, onProgress }: Props) {
  ...
  const [cameraPos, setCameraPos] = useState(new THREE.Vector3(-6.00, 0.84, 5.12))
  const [cameraTarget, setCameraTarget] = useState(new THREE.Vector3(-2.33, 0.20, -0.19))
  ...
  // 贴图模型层
  const [modelPx, setModelPx] = useState(-1.54); const [modelPy, setModelPy] = useState(-0.02); const [modelPz, setModelPz] = useState(-0.31)
  const [modelRx, setModelRx] = useState(0); const [modelRy, setModelRy] = useState(0); const [modelRz, setModelRz] = useState(0)
  const [modelScale, setModelScale] = useState(1.19)

  // 粒子层
  const [partPx, setPartPx] = useState(-1.54); const [partPy, setPartPy] = useState(-0.02); const [partPz, setPartPz] = useState(-0.31)
  const [partRx, setPartRx] = useState(0); const [partRy, setPartRy] = useState(0); const [partRz, setPartRz] = useState(0)
  const [partScale, setPartScale] = useState(1.19)
  ...
  const REAL_IMAGE = 'images/程阳永济桥实景图.jpg'
  ...
  const onSwipeEnd = useCallback((containerWidth: number) => {
    ...
    setDragOffset(0)
  }, [dragging, dragOffset, showRealImage])

  // 计算裁剪比例：场景图在上层，用 clip-path 从左侧裁剪
  // 初始 state: 场景图完全显示 (clipLeft = 0)
  // 完全显示实景图: 场景图完全被裁掉 (clipLeft = 100%)
    ...
    <div
      ...
      >
        <Canvas
          ...
          camera={{ position: [-6.00, 0.84, 5.12], fov: 42 }}
          dpr={[1, 2]}
      ...
      {/* ═══ 滑动提示箭头 ═══ */}
      {showHint && !showRealImage && !dragging && (
        <div
          style={{
            position: 'absolute',
            left: 8,
